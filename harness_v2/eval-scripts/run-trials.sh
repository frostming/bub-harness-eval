...
while IFS= read -r entry || [ -n "$entry" ]; do
  ...
  if [ -f "$trial_dir/trial.json" ] && jq -e '.recovery == true' "$trial_dir/trial.json" >/dev/null; then
    ...
    if [ -f "$RUN_DIR/controller/provider-blocked.json" ]; then
      echo "Provider quota blocked; stopping before a fresh attempt. Existing recovery=true cells can still be collected." >&2
      exit 1
    fi
    if [ -d "$trial_dir" ]; then
    ...
    mkdir -p "$trial_dir"
    # Leave at least one 4-vCPU / 8-GiB slot below the shared tenant limits.
    # This run uses the approved 4-vCPU / 8-GiB checkpoint for every task.
    if ! runta ps -a --json | jq -e '
      .runtimes as $r
      | all($r[]; (.vcpus | type) == "number" and (.max_memory_mib | type) == "number")
      and (([$r[].vcpus] | add // 0) + 4 <= 28)
      and (([$r[].max_memory_mib] | add // 0) + 8192 <= 57344)' >/dev/null; then
      echo "Shared runtime capacity unavailable or unknown; stopping before restore" >&2
      exit 1
    fi
    printf '\n=== [%s] restoring %s\n' "$entry" "$CHECKPOINT" >&2
  ...
  if [ "$resume" = false ]; then
    ...
    if [ -n "$SECRET_NAME" ] && [ -n "$SECRET_HOST" ]; then
      ...
      credential_template='Bearer ${secret}'
      if [ "$MODEL" = anthropic:k3 ]; then
        credential_header=x-api-key
        credential_template='${secret}'
      fi
      if ! retry_transport runta secret rule set "$runtime" --secret "$SECRET_NAME" --host "$SECRET_HOST" \
        --header "$credential_header" --template "$credential_template" >>"$trial_dir/restore.log" 2>&1; then
  ...
  mv "$trial_dir/trial.json.tmp" "$trial_dir/trial.json"
  python3 "$SCRIPT_DIR/calculate-cost.py" --trial "$trial_dir" --model "$MODEL" --harness "$HARNESS" --score --write
  if python3 - "$trial_dir/jobs" <<'PY'
import sys
from pathlib import Path
found = any("You've reached your 5-hour usage limit" in p.read_text(errors="replace")
            for p in Path(sys.argv[1]).rglob("bub-output.txt"))
sys.exit(0 if found else 1)
PY
  then
    mkdir -p "$RUN_DIR/controller"
    jq -n --arg task "$entry" --arg model "$MODEL" \
      '{provider:"Kimi Coding", model:$model, task:$task,
        reason:"HTTP 403: 5-hour usage limit reached", reset_at:null,
        resume_rule:"Confirm quota reset before removing this marker and resuming."}' \
      > "$RUN_DIR/controller/provider-blocked.json.tmp.$$"
    mv "$RUN_DIR/controller/provider-blocked.json.tmp.$$" "$RUN_DIR/controller/provider-blocked.json"
    echo "Kimi quota exhausted; other workers will stop before their next fresh attempt." >&2
  fi
